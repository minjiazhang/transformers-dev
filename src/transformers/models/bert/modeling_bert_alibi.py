...
class BertEncoderAlibi(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.slopes = torch.Tensor(get_slopes(self.attn_heads)).cuda()*-1
        self.alibi = self.slopes.unsqueeze(1).unsqueeze(1) * relative_position
        self.alibi = self.alibi.view(1, self.attn_heads, self.maxpos, self.maxpos)
