...
class BertEncoderAlibiNonsymNomask(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        slopes_left = torch.nn.parameter.Parameter(torch.Tensor(self.attn_heads))
        nn.init.normal_(slopes_left, -2,1)
        slopes_right = torch.nn.parameter.Parameter(torch.Tensor(self.attn_heads))
        nn.init.normal_(slopes_right, -2,1)

        slopes_left = -torch.sigmoid(slopes_left)
        slopes_right = -torch.sigmoid(slopes_right)

        context_position = torch.arange(self.maxpos)[:, None]
        memory_position = torch.arange(self.maxpos)[None, :]
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (
            ...
            self._future_mask = self._future_mask.unsqueeze(0) + self.alibi.cuda()
            # Do not add nonsym_mask tensor (option 3)
